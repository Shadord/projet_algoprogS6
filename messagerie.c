...
void set(Graph *G, LetterBox *LB, int jour_courant, int jour_vacc, float parametres[]) { // verifie l'état des personnes et envoie un message
	for (int i=0; i< G->nb_personnes; i++)
		...
		if (P->etat==0 && (jour_courant >= jour_vacc) && jour_vacc != -1) // si la personne est saine et que c'est le jour de campagne de vaccination
			{
			send_message(LB, send_Sain(P,P, parametres));
			}
		if (P->etat == 1) // si la personne est malade
			{
			send_message(LB, send_Malade(P,P, parametres)); // envoie un message à lui-même
			Successeur* S = G->liste_successeurs[i];
			while (S != NULL) // on envoie un message à tous les voisins
				{
				send_message(LB, send_Malade(P, S->personne, parametres));
				S = S->successeur;
				}
			}
		if (P->etat==2) // si la personnne est immunisée
			...
			while (S!=NULL) // on envoie un message à tous les voisins
				{
				send_message(LB, send_Immunise(P, S->personne, parametres));
				S=S->successeur;
				}
			}
		if (P->etat==4) // si la personne est zombie
			...
			while (S!=NULL) // on envoie un message à tous les voisins
				{
				send_message(LB, send_Zombie(P, S->personne, parametres));
				S=S->successeur;
				}
			}
		}
